...
- **近日公開セクション**: /about・/services・/faq への導線と告知カード

### 🧭 ナビゲーション
- **レスポンシブ対応**: モバイル・デスクトップ
- **メニュー項目**: ビジョン、ミッション、事業内容、バリュー、vibe経営、近日公開
- **スムーススクロール**: アンカーリンク
...
- **通知**: Resend APIでメール送信（未設定時は安全なログ出力にフォールバック）
...
## 📧 メール送信の設定（Resend 無料プラン）

1. [Resend](https://resend.com/) に無料登録し、ダッシュボードで API キーを発行します（Free Tier は1日あたり100通まで送信可能）。
2. 送信元メールアドレスを設定します。独自ドメインが未検証の場合は、提供される `onboarding@resend.dev` を利用できます。
3. プロジェクトルートにある `.env.example` を `.env.local` にコピーし、以下の環境変数を設定します。
   - `RESEND_API_KEY`: 発行したAPIキー
   - `RESEND_FROM_EMAIL`: 送信元メールアドレス（例: `onboarding@resend.dev`）
   - `CONTACT_TO_EMAIL`: 通知を受け取りたいメールアドレス
4. Resend SDKの動的レンダリングで使用する `@react-email/render` をインストールします（`npm install @react-email/render`）。
5. 開発サーバーを再起動すると `/api/contact` から実際にメールが送信されます。

> **メモ**: APIキーを設定していない場合でも、フォーム送信は安全にログ出力へフォールバックするため、開発中に誤送信する心配はありません。
